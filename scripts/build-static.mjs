...
const assetSets={gold:['ssb-gold-v1.ico','ssb-gold-v1-64.png','ssb-gold-v1-128.png','ssb-gold-touch-v1-180.png','manifest-gold-v1.json'],silver:['favicon.ico','ssb-favicon-v3-64.png','ssb-favicon-v3-128.png','ssb-touch-v3-180.png','manifest.json']};
...
for(const [buildName,build] of Object.entries(config)){
  ...
  for(const asset of [...sharedAssets,...(assetSets[buildName]||[])]){const source=path.join(root,asset);if(await exists(source))await cp(source,path.join(destination,asset))}

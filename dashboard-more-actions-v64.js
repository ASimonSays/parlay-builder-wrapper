/* DASHBOARD MORE ACTIONS V65 — compact primary row with expandable secondary actions */
(() => {
  'use strict';
  ...
  function validStructure(actions){
    return Boolean(actions?.querySelector(':scope > .savedActionsPrimary')&&actions.querySelector(':scope > .savedActionsDrawer')&&actions.querySelector('.savedActionsMoreToggle'));
  }

  function enhance(card){
    const actions=card.querySelector('.savedActions');
    if(!actions)return;
    if(validStructure(actions)){actions.dataset.moreReady='1';return}

    actions.dataset.moreReady='0';
    ...
    if(!view||!copy||!share||secondary.length<4)return;

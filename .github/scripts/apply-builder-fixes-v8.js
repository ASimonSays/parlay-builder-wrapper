...
.leg .headControls{display:flex;align-items:center;justify-content:flex-end;gap:14px;min-width:54px}
...
body.reorderMode .legHead{position:relative;display:grid;grid-template-columns:minmax(0,1fr) 66px minmax(92px,1fr);align-items:center;gap:14px;margin:0;min-height:42px}
body.reorderMode .legHeadLeft{min-width:0}
body.reorderMode .leg .headControls{display:contents}
body.reorderMode .leg .grip{display:inline-flex!important;grid-column:2;justify-self:center;width:64px;height:36px;cursor:grab;touch-action:none;-webkit-user-select:none;user-select:none}
body.reorderMode .leg .miniRemove{display:inline-flex!important;grid-column:3;justify-self:stretch;width:100%;min-width:92px;height:36px;padding:0 12px}
...
const originalAddLegV8=window.addLeg;window.addLeg=function(){originalAddLegV8();requestAnimationFrame(()=>applyReorderState())};
const originalRemoveLegV8=window.removeLeg;window.removeLeg=function(btn){originalRemoveLegV8(btn);requestAnimationFrame(()=>applyReorderState())};
const reorderObserverV8=new MutationObserver(()=>requestAnimationFrame(applyReorderState));
window.addEventListener('load',()=>{ensureReorderToolbar();applyReorderState();const legs=document.getElementById('legs');if(legs)reorderObserverV8.observe(legs,{childList:true})});
...
s = s.replace(/\/\* BUILDER_REORDER_MODE_V7 \*\/[\s\S]*?(?=<\/style>)/, css);
s = s.replace(/\/\* BUILDER_REORDER_MODE_V7 \*\/[\s\S]*?(?=<\/script>)/, js);
if(!s.includes('BUILDER_REORDER_MODE_V8')) throw new Error('V8 patch was not applied');
